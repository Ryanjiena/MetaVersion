...
chrome_plus_local_ver="$(cat ../shuax | grep -Po "chrome_plus-x64-v[\d.]+\.zip" | sed 's/chrome_plus-x64-v//g;s/\.zip//g' | tail -1)"
edge_plus_local_ver="$(cat ../shuax | grep -Po "edge_plus-x64-v[\d.]+\.zip" | sed 's/edge_plus-x64-v//g;s/\.zip//g' | tail -1)"
